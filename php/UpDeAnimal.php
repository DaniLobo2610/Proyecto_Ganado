...
<head>
    ...
    <title>Vender o eliminar Ganado</title>
...
<header>FINCA MACHADO LAMBURG</header>
 ...
 <img src="../images/Toro.png" alt="Logo de Proyecto">
     ...
     <h2>¿Eliminaras parte del ganado?</h2>
     
     <form action="UpDeAnimal.php" method="POST" onsubmit="return pedirContrasena();">
...
<select name="Dueño" class="input-field" required><br>
        <option selected disabled>--Seleccionar a quien le dará el ganado--</option>
        <?php
include("conexion.php");
// Consulta para obtener las pólizas
$sql = $conexion->query("SELECT * FROM usuario");
while ($resultado = $sql->fetch_assoc()) {
    echo "<option value='" . $resultado['ID'] . "'>" . $resultado['Nombre']  . "  " .  $resultado['Apellido'] . "</option>";
}
?>
</select>
...
$idNuevoDueno = $_POST['Dueño'] ?? ''; // puede estar vacío
$passwordIngresada = $_POST['password_check'] ?? '';


if ($idAnimal <= 0 || empty($passwordIngresada)) {
...
if ($idNuevoDueno === '') {
    // Eliminar vacunas primero
$stmt = $conexion->prepare("DELETE FROM vacunas WHERE IdAnimal = ?");
$stmt->bind_param("i", $idAnimal);
$stmt->execute();
$stmt->close();

$stmt = $conexion->prepare("DELETE FROM inseminaartificial WHERE IdAnimal = ?");
$stmt->bind_param("i", $idAnimal);
$stmt->execute();
$stmt->close();

$stmt = $conexion->prepare("DELETE FROM tratamiento WHERE IdAnimal = ?");
$stmt->bind_param("i", $idAnimal);
$stmt->execute();
$stmt->close();
// Luego eliminar el animal
$stmt = $conexion->prepare("DELETE FROM animales WHERE ID = ?");
$stmt->bind_param("i", $idAnimal);
if ($stmt->execute()) {
    echo "<script>alert('Animal eliminado correctamente'); window.location.href='UpDeanimal.php';</script>";
} else {
    echo "Error al eliminar animal: " . $stmt->error;
}
$stmt->close();

} else {
    
    $idNuevoDueno = intval($idNuevoDueno);
    $stmt = $conexion->prepare("UPDATE animales SET IdUser = ? WHERE ID = ?");
    $stmt->bind_param("ii", $idNuevoDueno, $idAnimal);
    if ($stmt->execute()) {
        echo "<script>alert('Dueño actualizado correctamente'); window.location.href='UpDeanimal.php';</script>";
    } else {
